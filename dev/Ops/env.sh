...
export CODOS_PATH=$(grep -o '"codosPath"[[:space:]]*:[[:space:]]*"[^"]*"' "$HOME/.codos/paths.json" | cut -d'"' -f4)
export VAULT_PATH=$(grep -o '"vaultPath"[[:space:]]*:[[:space:]]*"[^"]*"' "$HOME/.codos/paths.json" | cut -d'"' -f4)
export USER_NAME=$(grep -o '"userName"[[:space:]]*:[[:space:]]*"[^"]*"' "$HOME/.codos/paths.json" | cut -d'"' -f4)
...
# Load API keys (set -a exports all variables)
# Try bundle location first, then dev source tree
if [ -f "$HOME/Library/Application Support/com.codos.desktop/secrets.env" ]; then
    set -a
    source "$HOME/Library/Application Support/com.codos.desktop/secrets.env"
    set +a
elif [ -f "$CODOS_PATH/dev/Ops/.env" ]; then
    set -a
    source "$CODOS_PATH/dev/Ops/.env"
    set +a
fi
