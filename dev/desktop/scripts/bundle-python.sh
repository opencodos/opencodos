...
cp "$CODOS_ROOT/backend/__init__.py" "$SERVICES_DIR/backend/"
cp "$CODOS_ROOT/backend/__main__.py" "$SERVICES_DIR/backend/"

for mod in codos_utils codos_models codos_adapters codos_usecases codos_services; do
    rsync -a \
        --exclude '.venv' \
        --exclude '__pycache__' \
        --exclude 'tests' \
        --exclude '*.pyc' \
        "$CODOS_ROOT/backend/$mod" "$SERVICES_DIR/backend/"
done
