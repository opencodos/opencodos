...
script -q "$SUBMIT_LOG" xcrun notarytool submit "$ZIP_FILE" --keychain-profile "Codos notarize" --wait 2>&1
...
script -q "$DMG_LOG" xcrun notarytool submit "$DMG_FINAL" --keychain-profile "Codos notarize" --wait 2>&1
