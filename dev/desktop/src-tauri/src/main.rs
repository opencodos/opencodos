...
const SERVICE_CATALOG: [ManagedServiceConfig; 4] = [
    ManagedServiceConfig {
        ...
        command: ServiceCommand::PythonModule("backend", &["gateway"]),
        bundle_command: None,
        autostart: true,
    ...
    ManagedServiceConfig {
        ...
        cwd_relative: ".",
        bundle_relative: "services",
        command: ServiceCommand::PythonModule("backend", &["codos-bot"]),
        bundle_command: None,
        autostart: false,
...
const GATEWAY_BACKEND_SERVICE_ID: &str = "gateway-backend";
    ...
    let updated_settings = {
        ...
        if let Some(backend_mode) = patch.backend_mode {
            ...
            if backend_mode == BackendMode::AttachExisting {
                if let Some(runtime) = state.services.get_mut(GATEWAY_BACKEND_SERVICE_ID) {
...
fn start_service_inner(
    ...
    if service_id == GATEWAY_BACKEND_SERVICE_ID && state.runtime_network.attach_existing_backend {
        let runtime = state
            .services
            .get_mut(service_id)
            .ok_or_else(|| format!("unknown service: {service_id}"))?;
    ...
    if service_id == GATEWAY_BACKEND_SERVICE_ID
...
fn build_service_command(
    ...
    let python_path = if is_bundle_mode(app) {
        ...
        codos_root.to_path_buf() // {codos_root}
    };
    ...
    if is_bundle_mode(app) {
        ...
        let codos_data_dir = home_dir().unwrap_or_default().join(".codos");
        command.env("CODOS_DATA_DIR", &codos_data_dir);
    ...
    if runtime.config.id == GATEWAY_BACKEND_SERVICE_ID {
        command
            .env(
                "ATLAS_BACKEND_PORT",
...
fn service_status_from_runtime(
    ...
    let managed_externally =
        runtime.config.id == GATEWAY_BACKEND_SERVICE_ID && runtime_network.attach_existing_backend;
...
fn resolve_codos_root() -> Result<PathBuf, String> {
    ...
    if let Ok(explicit_root) = std::env::var("CODOS_ROOT") {
        ...
        if root
            .join("backend/codos_services/gateway/server.py")
            .exists()
        {
            return Ok(root);
        }
    ...
    for root in roots_to_probe {
        for candidate in root.ancestors() {
            let marker = candidate.join("backend/codos_services/gateway/server.py");
            if marker.exists() {
...
fn app_config_json_path() -> PathBuf {
    home_dir()
        .unwrap_or_default()
        .join(".codos")
        .join("config.json")
...
fn ensure_atlas_api_key(app: &AppHandle) -> Result<String, String> {
    let config_path = app_config_json_path();

    // Try to read existing key from config.json
    if config_path.exists() {
        if let Ok(content) = fs::read_to_string(&config_path) {
            if let Ok(json) = serde_json::from_str::<serde_json::Value>(&content) {
                if let Some(key) = json.get("atlas_api_key").and_then(|v| v.as_str()) {
                    if !key.is_empty() {
    ...
    json["atlas_api_key"] = serde_json::Value::String(key.clone());
        ...
        .setup(|app| {
            ...
            if let Ok(mut state) = lock_result {
                ...
                const ALWAYS_START: [&str; 2] =
                    [GATEWAY_BACKEND_SERVICE_ID, TELEGRAM_AGENT_SERVICE_ID];
                if should_autostart {
