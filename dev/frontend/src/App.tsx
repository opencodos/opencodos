...
function App() {
  ...
  useEffect(() => {
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/c518f0fb-a1d2-46f9-8bd7-c5265c0afef1',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'App.tsx:13',message:'App mounted',data:{hash:window.location.hash},timestamp:Date.now(),sessionId:'debug-session',runId:'pre-fix',hypothesisId:'D'})}).catch(()=>{});
    // #endregion
  ...
  useEffect(() => {
    ...
    const maybeRedirectToSetup = async () => {
      ...
      try {
        ...
        const needsSetup = status.needs_setup === true || status.needsSetup === true
        if (needsSetup) {
          redirectTo('/setup')
          return
        }
