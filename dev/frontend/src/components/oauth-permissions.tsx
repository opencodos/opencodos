...
export function OAuthPermissions({ workflow, onClose, onSaved }: OAuthPermissionsProps) {
  ...
  // Stable string keys for array deps (avoids re-running memo on reference changes)
  const includeKey = workflowConfig.conversation_filters?.include?.join(',');
  const whitelistKey = backendWhitelist.join(',');

  const initialSelectedIds = useMemo(() => {
    // First check workflow config, then backend config
    const workflowInclude = includeKey ? includeKey.split(',') : [];
    if (workflowInclude.length > 0) {
      return new Set(workflowInclude);
    }
    // Fall back to backend config whitelist
    const whitelist = whitelistKey ? whitelistKey.split(',') : [];
    if (whitelist.length > 0) {
      return new Set(whitelist);
    }
    return new Set<string>();
  }, [includeKey, whitelistKey]);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- intentional: only react to initialization signals
      ...
      <CardHeader className="space-y-4 pb-4">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2 rounded-md bg-muted px-3 py-1.5">
            ...
            <span className="text-sm text-muted-foreground">Codos&apos;s Workspace</span>
          </div>
        </div>

        <div>
          <h2 className="text-xl font-semibold text-balance">
            Allow &quot;Codos&quot; to access these channels
          </h2>
        </div>
      </CardHeader>
