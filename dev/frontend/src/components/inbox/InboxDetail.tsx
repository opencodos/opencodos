...
import { ExternalLink, MessageSquare, Reply, Users, UserCheck, Check, Send, X, Loader2 } from 'lucide-react'
...
function AISuggestionCard({ suggestion, telegramId, lastMessageId, onAskCodos, onDismiss }: { suggestion: InboxSuggestion; telegramId: number | null; lastMessageId?: number; onAskCodos: () => void; onDismiss: () => void }) {
  ...
  const handleSend = async () => {
    ...
    const result = await inboxAPI.sendMessage(telegramId, draft, lastMessageId)
    if (result.success) {
      setSendState('sent')
    } else {
...
function HeuristicSuggestionCard({ conversation, telegramId, lastMessageId, onAskCodos }: { conversation: VaultConversation; telegramId: number | null; lastMessageId?: number; onAskCodos: () => void }) {
  const [replyOpen, setReplyOpen] = useState(false)
...
export function InboxDetail({ conversation }: InboxDetailProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a0f]">
      ...
      {!dismissed && (
        suggestion ? (
          <AISuggestionCard suggestion={suggestion} telegramId={conversation.telegram_id ? Number(conversation.telegram_id) : null} lastMessageId={conversation.last_message_id} onAskCodos={() => setChatOpen(true)} onDismiss={() => setDismissed(true)} />
        ) : suggestionsLoaded ? (
          <HeuristicSuggestionCard conversation={conversation} telegramId={conversation.telegram_id ? Number(conversation.telegram_id) : null} lastMessageId={conversation.last_message_id} onAskCodos={() => setChatOpen(true)} />
        ) : null
      )}
