...
// eslint-disable-next-line no-control-regex -- intentional ANSI control character matching
    ...
    const loadMessages = async () => {
      try {
        ...
        if (session?.messages && session.messages.length > 0) {
          ...
          const loadedMessages: Message[] = (session.messages as unknown as ApiMessage[]).map((msg) => ({
            ...
            toolCalls: msg.tool_calls?.map((tc) => ({
              ...
              name: tc.name,
              status: (tc.status || 'complete') as ToolCall['status'],
              input: tc.input || {},
              output: tc.output,
  ...
  const handleHookEvent = useCallback((event: HookEvent) => {
    ...
    switch (hookEvent) {
      ...
      case 'Error':
        ...
        currentAssistantIdRef.current = null
        accumulatedContentRef.current = ''
        // Add error as a visually distinct message so the user can see it
                ...
                {msg.content && (
                  <div
                    className={`rounded-2xl px-4 py-3 ${
                      msg.isError
                        ? 'bg-red-500/10 text-red-200 rounded-tl-md border-l-2 border-red-500/60'
                        : msg.role === 'user'
                  ...
                  >
                    {msg.isError && (
                      <div className="flex items-center gap-1.5 mb-1.5 text-red-400 text-xs font-medium uppercase tracking-wide">
