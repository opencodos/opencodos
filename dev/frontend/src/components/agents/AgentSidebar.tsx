...
import { agentSessionsAPI, agentConfigAPI, inboxAPI } from '@/lib/api'
...
export function AgentSidebar({ activeAgent, onAgentSelect, onNewChat, onSessionSelect }: AgentSidebarProps) {
  ...
  // Fetch unread chat count for inbox badge
  useEffect(() => {
    const fetchUnread = () => {
      inboxAPI.getConversations()
        .then(data => setUnreadChats(data.conversations.filter(c => c.unread_count > 0).length))
        .catch(() => {})
    }
    fetchUnread()
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- only trigger on expand toggle
  }, [vaultExpanded])
                ...
                sessions
                  ...
                  .map((session) => {
                    ...
                    return (
                      <button
                        key={session.id}
                        onClick={() => onSessionSelect?.(session.id, session.agent_id)}
