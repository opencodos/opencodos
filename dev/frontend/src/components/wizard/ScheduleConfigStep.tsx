...
export function ScheduleConfigStep({ connectors, onComplete, onSkip }: ScheduleConfigStepProps) {
  ...
  useEffect(() => {
    const fetchPresets = async () => {
      try {
        ...
        // Initialize selections with defaults for connectors that have presets
  ...
  const handleInstall = useCallback(async () => {
    ...
    try {
      // Only include connectors that have scheduling support and valid selections
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {connectors.map((connectorId) => {
          const connectorPresets = getConnectorPresets(connectorId)
          const isComingSoon = !hasSchedulingSupport(connectorPresets)
          ...
          return (
            <div
              key={connectorId}
              className={`p-4 bg-black/30 rounded-xl border ${
                isComingSoon ? 'border-atlas-border/50 opacity-60' : 'border-atlas-border'
              }`}
            >
              {/* Connector header */}
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500">
                    {isComingSoon
                      ? 'Automatic scheduling not yet available'
              ...
              {/* Preset buttons or Coming Soon message */}
              {isComingSoon ? (
                <div className="flex items-center gap-2 text-sm text-gray-500">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <span>Manual sync only for now. You can trigger syncs from the dashboard.</span>
                </div>
              ) : (
      ...
      <p className="text-xs text-gray-500 text-center">
        Schedules are installed as macOS LaunchAgents and run in the background.
        You can modify them later from Settings.
      </p>
    </div>
  )
}
