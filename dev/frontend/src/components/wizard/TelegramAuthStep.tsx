...
export function TelegramAuthStep({
  ...
  const initiateAuth = useCallback(async (force = false) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Parse backend error detail for credential issues
        if (res.status === 400) {
          try {
            const errData = await res.json()
            const detail = errData.detail || ''
            if (detail.includes('TELEGRAM_API_ID') || detail.includes('TELEGRAM_API_HASH')) {
  ...
  useEffect(() => {
    ...
    void hydrate()
  }, [telegramEnabled, telegramStatus, initiateAuth, setTelegramStatus])
  ...
  return (
    <div className="space-y-6">
      ...
      {credsMissing && (
        <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-lg space-y-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-red-400 font-medium">Telegram API credentials missing</p>
              <p className="text-red-400/70 text-sm">Go back to the API Keys step and enter your Telegram API ID and Hash.</p>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Error */}
      {error && !agentNotRunning && !credsMissing && (
