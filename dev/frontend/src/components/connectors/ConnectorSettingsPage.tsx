...
export function ConnectorSettingsPage({ service, onBack }: ConnectorSettingsPageProps) {
  ...
  useEffect(() => {
    ...
    // Fetch telegram config to get accurate channel count
    if (service === 'telegram') {
            ...
            <div className="border rounded-md bg-white border-[#E5E5E5]">
              <div className="flex items-center justify-between py-2 px-3 gap-1.5">
                <div className="flex items-center gap-1.5">
                  {(() => {
                    // For Telegram, use backend config count; for others, use workflow config
                    const channelCount = service === 'telegram' && telegramChannelCount !== null
                      ? telegramChannelCount
                      : getChannelCount(workflow.config);
                    const isActive = channelCount > 0;
                    return (
                      <div className="relative flex items-center justify-center mx-2">
                        {/* Pulsing ring animation (only when active) - 12px container scales to 24px */}
                        {isActive && (
                          <span
                  ...
                  <div>
                    <span className="text-sm font-normal leading-5">
                      Connected {service === 'telegram' && telegramChannelCount !== null
                        ? telegramChannelCount
                        : getChannelCount(workflow.config)} channels
                    </span>
      ...
      {hasWorkflowSupport && workflow && (
        <WorkflowDetailModal
          ...
          onSave={() => {
            ...
            // Refresh telegram channel count from backend
            if (service === 'telegram') {
              integrationAPI.getTelegramConfig().then((config) => {
                setTelegramChannelCount(config.whitelist_ids.length);
              });
