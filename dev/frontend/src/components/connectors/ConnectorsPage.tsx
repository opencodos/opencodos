...
import { integrationAPI, workflowAPI, ensureWorkflow } from '@/lib/api';
...
// Services that connect via claude.ai Connectors (MCP)
const CLAUDE_AI_MCP_SERVICES = new Set(['slack', 'notion', 'linear']);

// Services that connect via gog CLI (Google Workspace)
const GOG_SERVICES = new Set(['google']);
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="space-y-2">
            ...
            <p className="text-base font-normal leading-5 text-[#737373]">
              You can connect your Codos account to other apps and services.
            </p>
          </div>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-6">
          {filteredIntegrations.map((integration) => {
            ...
            const isWorkflowService = integration.service === 'telegram' || integration.service === 'slack';
            const isClaudeAiMcp = connectionDetails?.account_id === 'claude-ai-mcp';
            const isGogService = GOG_SERVICES.has(integration.service);
              ...
              >
                ...
                <div className="flex flex-col gap-3 mt-4">
                  {!isSupported ? (
                    ...
                    isClaudeAiMcp ? (
                      // Connected via claude.ai MCP (e.g. Slack)
                      <div className="flex items-center justify-between border rounded-md px-3 py-2.5">
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-1.5">
                            <div className="h-2 w-2 rounded-full bg-green-500" />
                            <span className="text-sm font-medium text-foreground">Managed via claude.ai</span>
                          </div>
                          <span className="text-xs text-muted-foreground ml-3.5">
                            Manage in claude.ai Settings
                          </span>
                        </div>
                      </div>
                    ) : isGogService ? (
                      // Connected via gog CLI (Google Workspace)
                      <div className="flex items-center justify-between border rounded-md px-3 py-2.5">
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-1.5">
                            <div className="h-2 w-2 rounded-full bg-green-500" />
                            <span className="text-sm font-medium text-foreground">
                              Connected{connectionDetails?.account_id && connectionDetails.account_id !== 'google-account'
                                ? ` as ${connectionDetails.account_id}`
                                : ''}
                            </span>
                          </div>
                          <span className="text-xs text-muted-foreground ml-3.5">
                            Authenticated via gog CLI
                          </span>
                        </div>
                      </div>
                    ) : isWorkflowService ? (
                      // Telegram: Show channel count + Edit button
                      <div className="flex items-center justify-between border rounded-md px-3 py-2.5">
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-1.5">
                            <div className="h-2 w-2 rounded-full bg-green-500" />
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (integration.service === 'slack') {
                                ...
                                let wf = workflows.find(w => w.name.toLowerCase().includes('telegram ingestion'));
                                if (!wf) {
                                  wf = ensureWorkflow('telegram');
                                  fetchWorkflows(true);
                                }
                                openTelegramWorkflowModal(wf.id);
                              }
                            }}
                            className="gap-1.5"
                          >
                  ...
                  ) : GOG_SERVICES.has(integration.service) ? (
                    <div className="flex flex-col gap-1.5">
                      <span className="text-sm text-muted-foreground">
                        Run in terminal:
                      </span>
                      <code className="text-xs bg-muted px-2 py-1.5 rounded font-mono select-all">
                  ...
                  ) : CLAUDE_AI_MCP_SERVICES.has(integration.service) ? (
                    <div className="flex flex-col gap-1.5">
                      <Button
                      ...
                      <span className="text-xs text-muted-foreground">
                        Connect in claude.ai Settings, then refresh this page
                      </span>
                    </div>
                  ) : (
                    <Button
