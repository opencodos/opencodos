...
const CLAUDE_AI_MCP_SERVICES = new Set(['slack', 'notion', 'linear', 'gmail', 'googlecalendar']);

// Services detected via local file (no connect flow — install the app)
const LOCAL_SERVICES = new Set(['granola']);
...
export function ConnectorsPage() {
  ...
  const handleConnect = async (service: string) => {
    try {
      ...
      setError(`No connect flow for ${service}. Use claude.ai Settings > Integrations.`);
      setConnectingService(null);
    } catch (err) {
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col gap-3 mt-4">
                  ...
                  ) : LOCAL_SERVICES.has(integration.service) ? (
                    <span className="text-xs text-muted-foreground">
                      Install the {integration.name} app, then refresh this page
                    </span>
                  ) : (
                    <Button
