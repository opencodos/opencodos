      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {dep.required_version && dep.status === 'warning' && dep.logged_in !== false && (
        ...
        {dep.status === 'warning' && (
          <span className="text-xs text-yellow-400 font-medium">{dep.status_message || 'Update needed'}</span>
        )}
        {dep.status === 'missing' && (
  ...
  const checkDependencies = useCallback(async () => {
    ...
    try {
      ...
      setResult({
        all_ok: false,
        dependencies: [
          { name: 'bun', installed: false, version: null, required_version: '1.0', status: 'missing', install_command: 'curl -fsSL https://bun.sh/install | bash', logged_in: null, status_message: null },
          { name: 'claude', installed: false, version: null, required_version: '1.0', status: 'missing', install_command: 'curl -fsSL https://claude.ai/install.sh | bash', logged_in: null, status_message: null },
        ],
      })
    } finally {
      setChecking(false)
