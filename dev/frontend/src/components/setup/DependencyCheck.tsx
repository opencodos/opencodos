      ...
      <div className="flex items-center gap-3">
        ...
        {dep.status === 'warning' && (
          <span className="text-xs text-yellow-400 font-medium flex items-center gap-1">
            {dep.status_message || 'Update needed'}
            {dep.debug_info && (
              <span className="relative group cursor-help">
                <span className="text-gray-500 hover:text-gray-300">(&#63;)</span>
                <span className="absolute right-0 top-6 z-10 hidden group-hover:block w-80 p-2 bg-gray-900 border border-white/10 rounded text-xs text-gray-400 font-mono whitespace-pre-wrap break-all shadow-lg">
                  {dep.debug_info}
                </span>
              </span>
            )}
...
export function DependencyCheck({ onComplete, onSkip }: DependencyCheckProps) {
  ...
  // Auto-poll every 5s while dependencies aren't all ok
  useEffect(() => {
    if (allOk || checking) return
    const interval = setInterval(checkDependencies, 5000)
    return () => clearInterval(interval)
  }, [allOk, checking, checkDependencies])
