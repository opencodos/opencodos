...
def test_spawn_sync_not_called_from_server():
    ...
    server_path = repo_root / "backend" / "codos_services" / "telegram_agent" / "server.py"
    ...
    assert len(callers) == 0, (
        f"_spawn_sync() is called {len(callers)} time(s) — it should not be called anywhere to avoid lock contention"
    )
