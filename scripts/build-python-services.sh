...
#
# Builds: telegram-agent, atlas-bot, telegram-mcp
# Output: dev/desktop/src-tauri/resources/services/<name>/<name>
#
# connector-backend is excluded — it stays on bundled PYTHONHOME + server.py.
...
resolve_service() {
    ...
    case "$name" in
        telegram-agent)
            SVC_SRC_DIR="src/backend/telegram_agent"
            SVC_ENTRY="main.py"
            ;;
        atlas-bot)
            SVC_SRC_DIR="src/backend/atlas_bot"
            SVC_ENTRY="main.py"
            ;;
        telegram-mcp)
            SVC_SRC_DIR="src/backend/telegram_mcp"
            SVC_ENTRY="main.py"
            ;;
        *)
            echo "Unknown service: $name"
            echo "Available: telegram-agent, atlas-bot, telegram-mcp"
            return 1
            ;;
    esac
}
...
build_service() {
    ...
    local ingestion_dir="$REPO_ROOT/src/backend"
...
if [ $# -gt 0 ]; then
    ...
    for svc in telegram-agent atlas-bot telegram-mcp; do
        build_service "$svc"
    done
fi
