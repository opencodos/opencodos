...
**All MCP services run in a subprocess via `run-mcp.sh`.** This saves ~50k tokens of MCP tool schemas.
...
| `/notion` | Notion | `@notionhq/notion-mcp-server` (npm) | Search pages, get content, query databases |
...
### Pipedream-backed services

**Auth:** Pipedream Connect (OAuth tokens, account management).
**Data sync (ingestion):** Pipedream proxy API (`ingestion/lib/pipedream.ts`), with legacy Composio fallback.
**MCP tool access (`run-mcp.sh`):** Config files in `Dev/Ops/mcp/configs/` still point to Composio SSE endpoints (`mcp.composio.dev`). These are legacy but functional.

Skill hint files: `skills/Composio/<service>.md` (legacy name, still useful for tool discovery).

| Trigger | Service | What It Does |
|---------|---------|--------------|
| `/slack` | Slack | Send messages, list channels, fetch history |
| `/gmail` | Gmail | Read inbox, send email, search messages |
| `/calendar` | Calendar | List events, create meetings, find free slots |
| `/linear` | Linear | Create issues, track tasks, manage projects |
| `/github` | GitHub | List commits, search repos, view activity |
| `/gdrive` | Google Drive | Search files, download content, list folders |
| `/gdocs` | Google Docs | Read/create documents, insert text |
| `/gsheets` | Google Sheets | Read spreadsheets, lookup rows, add data |
...
- **Slack:** `run-mcp.sh slack "[task]"`
- **Email:** `run-mcp.sh gmail "[task]"`
