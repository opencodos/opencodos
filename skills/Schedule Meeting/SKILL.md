...
```bash
Dev/Ops/mcp/run-mcp.sh calendar "GOOGLECALENDAR_EVENTS_LIST for [start_date] to [end_date]. Look for flights, travel, timezone changes."
```
...
```bash
Dev/Ops/mcp/run-mcp.sh calendar "GOOGLECALENDAR_EVENTS_LIST for [target date]. Return start/end times."
```
...
**Slack:**
```bash
Dev/Ops/mcp/run-mcp.sh slack "SLACK_SEND_MESSAGE to channel [channel_id]: [message]"
```

**Gmail:**
```bash
Dev/Ops/mcp/run-mcp.sh gmail "GMAIL_SEND_EMAIL to [email] subject 'Reconnecting' body '[message]'"
```
...
```bash
Dev/Ops/mcp/run-mcp.sh calendar "GOOGLECALENDAR_CREATE_EVENT:
- Title: [Person's Name]//[Your Name]
- Start: [ISO datetime]
- End: [ISO datetime + 30min]
- Attendees: [email]
