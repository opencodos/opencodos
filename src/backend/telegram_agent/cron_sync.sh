...
cd "$CODOS_PATH/src/backend/telegram_agent" || exit 1
mkdir -p logs
...
if [[ "$HOUR" == "07" && "$MINUTE" -ge "40" && "$MINUTE" -le "50" ]]; then
    ...
    poetry run python daily_summary.py >> logs/summary.log 2>&1
...
poetry run python agent.py sync >> logs/cron.log 2>&1
