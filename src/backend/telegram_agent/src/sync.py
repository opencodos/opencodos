...
class SyncManager:
    ...
    async def sync(self) -> dict[str, int]:
        ...
        # Prune whitelist: keep only conversations that still have unreads
        # Use the full synced list (whitelist + discovered) as the source of truth
        unread_ids = {c.id for c in conversations if c.unread_count > 0}
        # Build current whitelist from all conversations we just synced (includes discovered)
        all_synced = {c.id: {"id": c.id, "name": c.name, "type": c.type} for c in conversations}
        pruned = [all_synced[cid] for cid in unread_ids if cid in all_synced]
        config_path = str(self.config.base_path / "config.yaml")
        prev_count = len(all_synced)
        if len(pruned) < prev_count:
            save_whitelist(config_path, pruned)
            console.print(f"[dim]Pruned whitelist: {prev_count} → {len(pruned)} conversations[/dim]")
