...
class SyncManager:
    ...
    def _load_context_sources(self, vault: Path) -> dict[str, list[tuple[str, str]]]:
        ...
        if granola_dir.exists():
            ...
            for f in sorted(granola_dir.glob("*.md"), reverse=True)[:60]:
                # Prepend filename so name-based matches work (e.g. "stepan" in "dima-stepan-sync")
                text = f"{f.stem}\n\n{f.read_text()[:3000]}"
    ...
    def _find_related_context(self, chat_name: str, sources: dict[str, list[tuple[str, str]]]) -> list[dict]:
        ...
        # "Dima, Gleb and Катя" -> ["Dima", "Gleb", "Катя"]
        # "stepan | slow response" -> ["stepan"]
        # "LayerZero <> Everclear (M&A)" -> ["LayerZero", "Everclear"]
    ...
    async def sync(self) -> dict[str, int]:
        ...
        # Prune whitelist: remove conversations with 0 unreads
        unread_ids = {c.id for c in conversations if c.unread_count > 0}
        current_whitelist = self.config.conversations.whitelist
        pruned = [c for c in current_whitelist if c["id"] in unread_ids]
        if len(pruned) < len(current_whitelist):
            save_whitelist(str(self.config.base_path / "config.yaml"), pruned)
            console.print(f"[dim]Pruned whitelist: {len(current_whitelist)} → {len(pruned)} conversations[/dim]")
