...
# Dev mode: derive codos root from this file's location
# src/backend/telegram_agent/src/config.py → 5 levels up to repo root
_CODOS_ROOT = Path(__file__).parent.parent.parent.parent.parent


def _reload_dotenv() -> None:
    """Re-read .env from disk. Called on every load_config() so keys saved
    after the agent started are picked up without a restart."""
    atlas_env = os.environ.get("ATLAS_ENV_FILE")
    if atlas_env:
        load_dotenv(atlas_env, override=True)
    else:
        load_dotenv(_CODOS_ROOT / "dev" / "Ops" / ".env", override=True)
...
def load_config(config_path: str = "config.yaml", require_anthropic: bool = True) -> Config:
    ...
    """
    # Re-read .env so keys saved after agent startup are visible
    _reload_dotenv()
    ...
    # Telegram config - prefer env vars
    api_id = os.environ.get("TELEGRAM_API_ID") or raw.get("telegram", {}).get("api_id")
    api_hash = os.environ.get("TELEGRAM_API_HASH") or raw.get("telegram", {}).get("api_hash")

    if not api_id or not api_hash:
        raise ValueError("TELEGRAM_API_ID and TELEGRAM_API_HASH must be set in .env or config.yaml")
    ...
    # Anthropic config - prefer env var
    api_key = os.environ.get("ANTHROPIC_API_KEY") or raw.get("anthropic", {}).get("api_key", "")
    ...
    # Obsidian config - prefer env var, fall back to config.yaml, then paths.json
    vault_path_str = os.environ.get("VAULT_PATH") or raw.get("obsidian", {}).get("vault_path")
    if not vault_path_str:
        # Fall back to paths.json (single source of truth for vault location)
        paths_json = Path.home() / ".codos" / "paths.json"
        if paths_json.exists():
            import json
            vault_path_str = json.loads(paths_json.read_text()).get("vaultPath")
    if vault_path_str:
        ...
        raise ValueError("VAULT_PATH not set in environment, config.yaml, or ~/.codos/paths.json")
