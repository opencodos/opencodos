...
def _spawn_suggestion_generation():
    ...
    # Resolve codos root from env or paths.json
    codos_path = os.environ.get("CODOS_PATH")
    if not codos_path:
        paths_file = Path.home() / ".codos" / "paths.json"
        if paths_file.exists():
            import json
            codos_path = json.loads(paths_file.read_text()).get("codosPath")
    if not codos_path:
        # Fallback: derive from source tree
        codos_path = str(Path(__file__).parent.parent.parent.parent)

    script = Path(codos_path) / "skills" / "Inbox Suggestions" / "run-inbox-suggestions.sh"
    if not script.exists():
        console.print(f"[dim]Suggestions script not found: {script}[/dim]")
        return

    log_dir = Path(_DATA_DIR) / "logs" / "telegram-sync"
