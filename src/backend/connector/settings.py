...
_DEFAULT_VAULT = str(Path.home() / "projects" / "codos_vault")  # Canonical default: ingestion/lib/paths.py
...
def _detect_source_tree_root() -> str | None:
    ...
    try:
        ...
        for parent in current.parents:
            if (parent / "skills").exists() and (parent / "src" / "backend" / "connector").exists():
                return str(parent)
    except Exception:
        pass
    return None
...
def _load_codos_path_from_paths_json() -> str | None:
    """Read codosPath from ~/.codos/paths.json (written by setup wizard)."""
    paths_file = ATLAS_CONFIG_DIR / "paths.json"
    if paths_file.exists():
        try:
            with open(paths_file) as f:
                paths = json.load(f)
            value = paths.get("codosPath")
            if value:
...
def _load_vault_path_from_paths_json() -> str | None:
    """Read vaultPath from ~/.codos/paths.json (written by setup wizard)."""
    paths_file = ATLAS_CONFIG_DIR / "paths.json"
    if paths_file.exists():
        try:
            with open(paths_file) as f:
                paths = json.load(f)
            value = paths.get("vaultPath") or paths.get("vault_path")
            if value:
                return str(Path(value).expanduser())
        except Exception:
            pass
...
class Settings(BaseSettings):
    ...
    pipedream_external_user_id: str | None = None
    pipedream_env: str = "production"
    ...
    def get_vault_path(self) -> Path:
        ...
        2. ~/.codos/paths.json vaultPath (set by setup wizard)
        3. Default: ~/projects/codos_vault
        """
