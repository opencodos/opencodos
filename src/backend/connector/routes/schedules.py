...
SCHEDULE_PRESETS: dict[str, ConnectorPresets] = {
    ...
    "slack": ConnectorPresets(
        ...
        presets=[
            SchedulePreset(
                id="default",
                label="Twice daily (7:45, 18:45)",
                description="Morning and evening sync",
                cron="45 7,18 * * *",
            ),
            SchedulePreset(
                id="frequent", label="Three times daily", description="Morning, noon, evening", cron="45 7,12,18 * * *"
            ),
            SchedulePreset(id="manual", label="Manual only", description="No automatic sync"),
        ],
    ),
    ...
    "notion": ConnectorPresets(
        ...
        presets=[
            ...
            SchedulePreset(id="hourly", label="Every hour", description="Low frequency", interval_minutes=60),
            SchedulePreset(id="manual", label="Manual only", description="No automatic sync"),
        ],
    ),
...
<dict>
    <key>Label</key>
    <string>com.atlas.process-cleanup</string>
...
<dict>
    <key>Label</key>
    <string>com.atlas.telegram-agent</string>
...
def _install_process_cleanup_plist() -> bool:
    ...
    plist_path = LAUNCH_AGENTS_DIR / "com.atlas.process-cleanup.plist"
...
def _generate_plist(connector: str, preset: SchedulePreset, codos_path: Path) -> str:
    ...
    label = f"com.atlas.{connector}-sync"
...
async def get_installed_schedules():
    ...
    if LAUNCH_AGENTS_DIR.exists():
        for plist_file in LAUNCH_AGENTS_DIR.glob("com.atlas.*.plist"):
            try:
    ...
    for connector, connector_presets in SCHEDULE_PRESETS.items():
        ...
        if installed:
            ...
            # Check if loaded
            label = plist_data.get("Label", f"com.atlas.{connector}-sync")
...
async def install_schedules(request: InstallRequest):
    ...
    for selection in request.selections:
        ...
        try:
            ...
            # Write plist file
            plist_path = LAUNCH_AGENTS_DIR / f"com.atlas.{selection.connector}-sync.plist"
...
async def uninstall_schedule(connector: str):
    ...
    plist_path = LAUNCH_AGENTS_DIR / f"com.atlas.{connector}-sync.plist"
...
async def install_telegram_agent_service():
    ...
    """
    codos_path = settings.get_codos_path()
    plist_path = LAUNCH_AGENTS_DIR / "com.atlas.telegram-agent.plist"
