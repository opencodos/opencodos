#!/usr/bin/env python3
"""
Atlas Bot - Telegram interface for Claude Code

Process name marker for safe killing: ATLAS_TG_BOT
...
from .formatter import format_response, format_session_info, split_message
...
configure_logging("atlas-bot", intercept_stdlib=False)
...
def transcribe_voice_sync(voice_bytes: bytes) -> str:
    ...
    try:
        ...
        transcript = transcriber.transcribe(voice_bytes, config=config)
...
async def analyze_image_async(image_bytes: bytes, media_type: str, user_prompt: str = None) -> str:
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if file_size > 10 * 1024 * 1024:
        await update.message.reply_text("⚠️ File too large (max 10MB)")
        return
    ...
    if is_text:
        try:
            ...
            await update.message.reply_text(f"⚠️ Could not read {file_name} as text")
            return
    else:
...
async def cmd_new(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🆕 New session started\nSession ID: `{new_session_id[:8]}...`", parse_mode="HTML")
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(format_session_info(info), parse_mode="HTML")
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_authorized(user_id):
        await update.message.reply_text(
            f"⛔ Unauthorized\n\nYour user ID: `{user_id}`\nAdd this to AUTHORIZED_USER_IDS in .env", parse_mode="HTML"
        )
        return
...
def test_claude():
    ...
    result = subprocess.run(
        [CLAUDE_BIN, "-p", "Say 'Hello from Atlas Bot!' in one line", "--model", CLAUDE_MODEL],
        capture_output=True,
        text=True,
        cwd=WORKSPACE_DIR,
        timeout=60,
...
def main():
    ...
    if not AUTHORIZED_USERS:
        print("WARNING: No AUTHORIZED_USER_IDS set in .env")
        print("The bot will reject all messages until configured.")

    logger.info("Starting Atlas Bot...")
