"""Slack configuration loader for connector-backend and sync scripts."""
...
from backend.codos_utils.paths import VAULT_PATH as _VAULT_PATH_STR

DEFAULT_VAULT_PATH = Path(_VAULT_PATH_STR)
ATLAS_PATHS_FILE = Path.home() / ".codos" / "paths.json"
...
def _load_vault_path() -> Path:
    """Resolve vault path from ~/.codos/paths.json with safe fallbacks."""
    if ATLAS_PATHS_FILE.exists():
        try:
            with open(ATLAS_PATHS_FILE, encoding="utf-8") as f:
                paths = json.load(f) or {}
            configured = paths.get("vaultPath") or paths.get("vault_path")
            if configured:
