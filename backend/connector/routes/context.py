...
def _get_cached_data() -> ContextResponse | None:
    ...
    if _is_cache_valid() and _context_cache["data"]:
...
def _build_full_context() -> ContextResponse:
    ...
    today = TodayContext(
        ...
        calls_date=calendar_calls["calls_date"] if calendar_calls["found"] else brief_data.get("morning_brief_date", ""),
        calls_is_fallback=calendar_calls["is_fallback"] if calendar_calls["found"] else brief_data.get("is_fallback", False),
