...
from backend.codos_models.settings import settings as _settings
...
TELEGRAM_API_ID = int(_settings.telegram_api_id)
...
try:
    configure_logging("telegram-mcp", level="ERROR", log_file=log_file_path, intercept_stdlib=False)
...
def log_and_format_error(
    function_name: str,
    error: Exception,
    prefix: ErrorCategory | str | None = None,
...
def format_entity(entity: Channel | Chat | User) -> dict[str, Any]:
    """Helper function to format entity information consistently."""
    result: dict[str, Any] = {"id": entity.id}
...
def get_sender_name(message) -> str:
    ...
    if isinstance(sender, (Channel, Chat)):
        return str(sender.title) if sender.title else "Unknown"
...
@validate_id("chat_id")
async def get_messages(chat_id: int | str, page: int = 1, page_size: int = 20) -> str:
    """
    ...
    try:
        ...
        lines = []
        for msg in messages:  # type: ignore  # get_messages returns iterable TotalList
...
@validate_id("chat_id")
async def send_message(chat_id: int | str, message: str) -> str:
    """
    ...
    try:
        entity = await client.get_entity(chat_id)
        await client.send_message(entity, message)  # type: ignore  # Telethon accepts this at runtime
...
@validate_id("channel")
async def subscribe_public_channel(channel: int | str) -> str:
    """
    Subscribe (join) to a public channel or supergroup by username or ID.
    """
    try:
        entity = await client.get_entity(channel)
        await client(functions.channels.JoinChannelRequest(channel=entity))  # type: ignore  # Telethon accepts this at runtime
        title = getattr(entity, "title", getattr(entity, "username", "Unknown channel"))
        return f"Subscribed to {title}."
    except telethon.errors.rpcerrorlist.UserAlreadyParticipantError:
        title = getattr(entity, "title", getattr(entity, "username", "this channel"))  # type: ignore  # variable assigned in try block above
...
@validate_id("chat_id")
async def list_inline_buttons(chat_id: int | str, message_id: int | str | None = None, limit: int = 20) -> str:
    """
    ...
    try:
        ...
        if message_id is not None:
            ...
            target_message = next((msg for msg in recent_messages if getattr(msg, "buttons", None)), None)  # type: ignore  # checked for None above
    ...
    try:
        ...
        callback_result = await client(
            functions.messages.GetBotCallbackAnswerRequest(
                peer=entity,  # type: ignore[arg-type]  # Telethon accepts this at runtime
        ...
        if getattr(callback_result, "message", None):
            response_parts.append(callback_result.message)  # type: ignore  # Telethon returns typed result
...
async def list_contacts() -> str:
    ...
    try:
        ...
        users = result.users  # type: ignore  # Telethon returns typed result
        if not users:
...
async def list_messages(
    ...
    try:
        ...
        if search_query:
            ...
            messages = []
            async for msg in client.iter_messages(entity, **params):  # type: ignore  # newest -> oldest; Telethon accepts this at runtime
            ...
            if from_date_obj or to_date_obj:
                ...
                if from_date_obj:
                    ...
                    async for msg in client.iter_messages(entity, offset_date=from_date_obj, reverse=True):  # type: ignore  # Telethon accepts this at runtime
                        if to_date_obj and msg.date > to_date_obj:
                            break
                    ...
                    async for msg in client.iter_messages(
                        ...
                        entity,  # type: ignore  # Telethon accepts this at runtime
                        offset_date=to_date_obj + timedelta(microseconds=1),  # type: ignore  # checked for None above
                    ):
                        messages.append(msg)
                        if len(messages) >= limit:
                            break
            else:
                messages = await client.get_messages(entity, limit=limit, **params)  # type: ignore  # get_messages returns TotalList which is list-like
        ...
        for msg in messages:  # type: ignore  # get_messages returns iterable TotalList
            ...
            lines.append(
                f"ID: {msg.id} | {sender_name} | Date: {msg.date}"
                f"{reply_info}{engagement_info} | Message: {message_text}"
            )
...
async def list_topics(
    ...
    try:
        ...
        result = await client(
            functions.channels.GetForumTopicsRequest(  # type: ignore  # Telethon returns typed result
                channel=entity,
                offset_date=0,
                offset_id=0,
        ...
        if getattr(result, "messages", None):
            messages_map = {message.id: message for message in result.messages}  # type: ignore  # Telethon returns typed result
...
async def list_chats(chat_type: str | None = None, limit: int = 20) -> str:
    ...
    try:
        ...
        for dialog in dialogs:
            ...
            if isinstance(entity, (Channel, User)) and entity.username:
...
@validate_id("chat_id")
async def get_chat(chat_id: int | str) -> str:
    """
    ...
    try:
        ...
        result = []
        result.append(f"ID: {entity.id}")  # type: ignore  # entity always has .id
...
async def get_direct_chat_by_contact(contact_query: str) -> str:
    ...
    try:
        ...
        result = await client(functions.contacts.GetContactsRequest(hash=0))
        contacts = result.users  # type: ignore  # Telethon returns typed result
        found_contacts = []
...
async def get_contact_chats(contact_id: int | str) -> str:
    """
    ...
    try:
        ...
        try:
            common = await client.get_common_chats(contact)  # type: ignore  # Telethon method exists at runtime
            for chat in common:
...
@validate_id("contact_id")
async def get_last_interaction(contact_id: int | str) -> str:
    """
...
@validate_id("chat_id")
async def get_message_context(chat_id: int | str, message_id: int, context_size: int = 3) -> str:
    """
    ...
    try:
        ...
        if central_message is not None and not isinstance(central_message, list):
            central_message = [central_message]  # type: ignore  # narrowing to single Message then wrapping in list
        elif central_message is None:
            central_message = []  # type: ignore  # empty list is compatible at runtime
        ...
        all_messages = list(messages_before) + list(central_message) + list(messages_after)  # type: ignore  # get_messages returns iterable TotalList
        ...
        for msg in all_messages:
            ...
            if msg.reply_to and msg.reply_to.reply_to_msg_id:
                try:
                    ...
                    if replied_msg:
                        ...
                        if replied_msg.sender:  # type: ignore[union-attr]  # get_messages(ids=) returns Message
                            replied_sender = getattr(
                                replied_msg.sender,  # type: ignore[union-attr]  # Message has sender
                                "first_name",
                                "",
                            ) or getattr(
                                replied_msg.sender,  # type: ignore[union-attr]
                                "title",
                                "Unknown",
                            )
                        reply_content = (
                            f" | reply to {msg.reply_to.reply_to_msg_id}\n"
                            f"  → Replied message: [{replied_sender}] "
                            f"{replied_msg.message or '[Media/No text]'}"  # type: ignore  # Message has message attr
                        )
                except Exception:
            ...
            results.append(
                ...
                f"{msg.message or '[Media/No text]'}\n"
            )
        return "\n".join(results)
    except Exception as e:
        return log_and_format_error(
...
async def add_contact(phone: str, first_name: str, last_name: str = "") -> str:
    ...
    try:
        ...
        if result.imported:  # type: ignore  # Telethon returns typed result
        ...
        try:
            result = await client(
                functions.contacts.ImportContactsRequest(
                    contacts=[
                        {  # type: ignore  # Telethon accepts dict as InputPhoneContact at runtime
                            "client_id": 0,
                            "phone": phone,
                            "first_name": first_name,
                            "last_name": last_name,
            ...
            if hasattr(result, "imported") and result.imported:  # type: ignore  # Telethon returns typed result
...
@validate_id("user_id")
async def delete_contact(user_id: int | str) -> str:
    """
    ...
    try:
        ...
        await client(functions.contacts.DeleteContactsRequest(id=[user]))  # type: ignore  # Telethon accepts this at runtime
...
@validate_id("user_id")
async def block_user(user_id: int | str) -> str:
    """
    ...
    try:
        user = await client.get_entity(user_id)
        await client(functions.contacts.BlockRequest(id=user))  # type: ignore  # Telethon accepts this at runtime
...
@validate_id("user_id")
async def unblock_user(user_id: int | str) -> str:
    """
    ...
    try:
        user = await client.get_entity(user_id)
        await client(functions.contacts.UnblockRequest(id=user))  # type: ignore  # Telethon accepts this at runtime
...
async def get_me() -> str:
    ...
    try:
        ...
        return json.dumps(format_entity(me), indent=2)  # type: ignore[arg-type]  # get_me() returns User at runtime
    except Exception as e:
...
async def create_group(title: str, user_ids: list[int | str]) -> str:
    """
    ...
    try:
        ...
        try:
            ...
            result = await client(functions.messages.CreateChatRequest(users=users, title=title))  # type: ignore  # Telethon accepts entity objects at runtime

            # Check what type of response we got
            if hasattr(result, "chats") and result.chats:  # type: ignore  # Telethon returns typed result
                created_chat = result.chats[0]  # type: ignore  # Telethon returns typed result
                return f"Group created with ID: {created_chat.id}"
            elif hasattr(result, "chat") and result.chat:  # type: ignore  # Telethon returns typed result
                return f"Group created with ID: {result.chat.id}"  # type: ignore  # Telethon returns typed result
            elif hasattr(result, "chat_id"):
                return f"Group created with ID: {result.chat_id}"  # type: ignore  # Telethon returns typed result
            else:
...
async def invite_to_group(group_id: int | str, user_ids: list[int | str]) -> str:
    """
    ...
    try:
        ...
        try:
            result = await client(functions.channels.InviteToChannelRequest(channel=entity, users=users_to_add))  # type: ignore  # Telethon accepts this at runtime

            invited_count = 0
            if hasattr(result, "users") and result.users:  # type: ignore  # Telethon returns typed result
                invited_count = len(result.users)  # type: ignore  # Telethon returns typed result
            elif hasattr(result, "count"):
                invited_count = result.count

            return f"Successfully invited {invited_count} users to {entity.title}"  # type: ignore  # Telethon returns typed result
        except telethon.errors.rpcerrorlist.UserNotMutualContactError:
            return (
                "Error: Cannot invite users who are not mutual contacts. "
                "Please ensure the users are in your contacts and have added you back."
            )
...
async def leave_chat(chat_id: int | str) -> str:
    ...
    try:
        ...
        if isinstance(entity, Channel):
            ...
            try:
                await client(functions.channels.LeaveChannelRequest(channel=entity))  # type: ignore  # Telethon accepts this at runtime
            ...
            try:
                ...
                await client(
                    functions.messages.DeleteChatUserRequest(
                        chat_id=entity.id,
                        user_id=me,  # type: ignore  # Telethon accepts entity ID directly at runtime
                    )
                )
                chat_name = getattr(entity, "title", str(chat_id))
                ...
                try:
                    ...
                    await client(functions.messages.DeleteChatUserRequest(chat_id=entity.id, user_id=me_full.id))  # type: ignore  # Telethon accepts this at runtime
                    chat_name = getattr(entity, "title", str(chat_id))
                    return f"Left basic group {chat_name} (ID: {chat_id})."
            ...
            return log_and_format_error(
                "leave_chat",
                Exception(
                    f"Cannot leave chat ID {chat_id} of type {entity_type}. "
                    "This function is for groups and channels only."
                ),
                chat_id=chat_id,
            )
        ...
        if "invalid" in error_str and "chat" in error_str:
            return log_and_format_error(
                "leave_chat",
                Exception(
                    "Error leaving chat: This appears to be a channel/supergroup. "
                    "Please check the chat ID and try again."
                ),
                chat_id=chat_id,
            )
...
@validate_id("chat_id")
async def get_participants(chat_id: int | str) -> str:
    """
...
@validate_id("chat_id")
async def send_file(chat_id: int | str, file_path: str, caption: str | None = None) -> str:
    """
    ...
    try:
        ...
        entity = await client.get_entity(chat_id)
        await client.send_file(entity, file_path, caption=caption)  # type: ignore  # Telethon accepts this at runtime
...
@mcp.tool(annotations=ToolAnnotations(title="Download Media", openWorldHint=True, readOnlyHint=True))
@validate_id("chat_id")
async def download_media(chat_id: int | str, message_id: int, file_path: str) -> str:
    """
    ...
    try:
        ...
        if not msg or not msg.media:  # type: ignore  # Telethon returns typed result
            return "No media found in the specified message."
        # Check if directory is writable
        dir_path = os.path.dirname(file_path) or "."
        if not os.access(dir_path, os.W_OK):
            return f"Directory not writable: {dir_path}"
        await client.download_media(msg, file=file_path)  # type: ignore  # Telethon returns typed result
        if not os.path.isfile(file_path):
...
async def update_profile(first_name: str | None = None, last_name: str | None = None, about: str | None = None) -> str:
    """
...
async def delete_profile_photo() -> str:
    ...
    """
    try:
        photos = await client(functions.photos.GetUserPhotosRequest(user_id="me", offset=0, max_id=0, limit=1))  # type: ignore  # Telethon accepts this at runtime
        if not photos.photos:  # type: ignore  # Telethon returns typed result
            return "No profile photo to delete."
        await client(functions.photos.DeletePhotosRequest(id=[photos.photos[0].id]))  # type: ignore  # Telethon returns typed result
    ...
    try:
        ...
        try:
            ...
            if "TLObject was expected" in str(e):
                return (
                    "Error: Privacy settings API call failed due to type mismatch. "
                    "This is likely a version compatibility issue with Telethon."
                )
            else:
                raise
    except Exception as e:
    ...
    try:
        ...
        if allow_users is None or len(allow_users) == 0:
            ...
            try:
                ...
                if allow_entities:
                    rules.append(InputPrivacyValueAllowUsers(users=allow_entities))  # type: ignore  # Telethon accepts entities at runtime
        ...
        if disallow_users and len(disallow_users) > 0:
            try:
                ...
                if disallow_entities:
                    rules.append(InputPrivacyValueDisallowUsers(users=disallow_entities))  # type: ignore  # Telethon accepts entities at runtime
        ...
        try:
            await client(functions.account.SetPrivacyRequest(key=privacy_key, rules=rules))  # type: ignore  # Telethon accepts these types at runtime
...
async def import_contacts(contacts: list) -> str:
    ...
    try:
        input_contacts = [
            functions.contacts.InputPhoneContact(  # type: ignore  # Telethon returns typed result
                client_id=i,
                phone=c["phone"],
        ...
        return f"Imported {len(result.imported)} contacts."  # type: ignore  # Telethon returns typed result
    except Exception as e:
...
async def get_blocked_users() -> str:
    ...
    try:
        ...
        return json.dumps([format_entity(u) for u in result.users], indent=2)  # type: ignore  # Telethon returns typed result
    except Exception as e:
...
async def create_channel(title: str, about: str = "", megagroup: bool = False) -> str:
    ...
    try:
        ...
        return f"Channel '{title}' created with ID: {result.chats[0].id}"  # type: ignore  # Telethon returns typed result
    except Exception as e:
...
@validate_id("chat_id")
async def edit_chat_title(chat_id: int | str, title: str) -> str:
    """
    ...
    try:
        entity = await client.get_entity(chat_id)
        if isinstance(entity, Channel):
            await client(functions.channels.EditTitleRequest(channel=entity, title=title))  # type: ignore  # Telethon accepts this at runtime
        elif isinstance(entity, Chat):
            await client(functions.messages.EditChatTitleRequest(chat_id=chat_id, title=title))  # type: ignore  # Telethon accepts this at runtime
        else:
...
@validate_id("chat_id")
async def edit_chat_photo(chat_id: int | str, file_path: str) -> str:
    """
    ...
    try:
        ...
        if isinstance(entity, Channel):
            ...
            await client(functions.channels.EditPhotoRequest(channel=entity, photo=input_photo))  # type: ignore  # Telethon accepts this at runtime
        elif isinstance(entity, Chat):
            # For basic groups, use EditChatPhotoRequest with InputChatUploadedPhoto
            input_photo = InputChatUploadedPhoto(file=uploaded_file)
            await client(functions.messages.EditChatPhotoRequest(chat_id=chat_id, photo=input_photo))  # type: ignore  # Telethon accepts this at runtime
        else:
...
@validate_id("chat_id")
async def delete_chat_photo(chat_id: int | str) -> str:
    """
    ...
    try:
        ...
        if isinstance(entity, Channel):
            ...
            await client(functions.channels.EditPhotoRequest(channel=entity, photo=InputChatPhotoEmpty()))  # type: ignore  # Telethon accepts this at runtime
        elif isinstance(entity, Chat):
            # Use None (or InputChatPhotoEmpty) for basic groups
            await client(functions.messages.EditChatPhotoRequest(chat_id=chat_id, photo=InputChatPhotoEmpty()))  # type: ignore  # Telethon accepts this at runtime
        else:
...
async def promote_admin(group_id: int | str, user_id: int | str, rights: dict[str, Any] | None = None) -> str:
    """
        ...
        try:
            await client(
                functions.channels.EditAdminRequest(
                    channel=chat,  # type: ignore[arg-type]  # Telethon accepts this at runtime
                    user_id=user,  # type: ignore[arg-type]
                    admin_rights=admin_rights,
                    rank="Admin",
                )
            )
            return f"Successfully promoted user {user_id} to admin in {chat.title}"  # type: ignore[union-attr]  # entity type narrowed by hasattr/isinstance
        except telethon.errors.rpcerrorlist.UserNotMutualContactError:
            return (
                "Error: Cannot promote users who are not mutual contacts. "
                "Please ensure the user is in your contacts and has added you back."
            )
        except Exception as e:
...
@validate_id("group_id", "user_id")
async def demote_admin(group_id: int | str, user_id: int | str) -> str:
    """
    ...
    try:
        ...
        try:
            ...
            return f"Successfully demoted user {user_id} from admin in {chat.title}"  # type: ignore[union-attr]  # entity type narrowed by hasattr/isinstance
        except telethon.errors.rpcerrorlist.UserNotMutualContactError:
            return (
                "Error: Cannot modify admin status of users who are not mutual contacts. "
                "Please ensure the user is in your contacts and has added you back."
            )
        except Exception as e:
...
async def ban_user(chat_id: int | str, user_id: int | str) -> str:
    """
        ...
        try:
            await client(
                functions.channels.EditBannedRequest(
                    channel=chat,  # type: ignore[arg-type]  # Telethon accepts this at runtime
                    participant=user,  # type: ignore[arg-type]
                    banned_rights=banned_rights,
                )
            )
            return f"User {user_id} banned from chat {chat.title} (ID: {chat_id})."  # type: ignore[union-attr]  # entity type narrowed by hasattr/isinstance
        except telethon.errors.rpcerrorlist.UserNotMutualContactError:
            return (
                "Error: Cannot ban users who are not mutual contacts. "
                "Please ensure the user is in your contacts and has added you back."
            )
        except Exception as e:
...
@validate_id("chat_id", "user_id")
async def unban_user(chat_id: int | str, user_id: int | str) -> str:
    """
    ...
    try:
        ...
        try:
            ...
            return f"User {user_id} unbanned from chat {chat.title} (ID: {chat_id})."  # type: ignore[union-attr]  # entity type narrowed by hasattr/isinstance
        except telethon.errors.rpcerrorlist.UserNotMutualContactError:
            return (
                "Error: Cannot modify status of users who are not mutual contacts. "
                "Please ensure the user is in your contacts and has added you back."
            )
        except Exception as e:
...
@validate_id("chat_id")
async def get_admins(chat_id: int | str) -> str:
    """
...
@validate_id("chat_id")
async def get_banned_users(chat_id: int | str) -> str:
    """
...
@validate_id("chat_id")
async def get_invite_link(chat_id: int | str) -> str:
    """
    ...
    try:
        ...
        try:
            ...
            result = await client(functions.messages.ExportChatInviteRequest(peer=entity))  # type: ignore  # Telethon accepts this at runtime
            return result.link  # type: ignore  # Telethon returns typed result
        ...
        try:
            invite_link = await client.export_chat_invite_link(entity)  # type: ignore  # Telethon method exists at runtime
        ...
        try:
            if isinstance(entity, (Chat, Channel)):
                full_chat = await client(functions.messages.GetFullChatRequest(chat_id=entity.id))  # type: ignore  # variable assigned in try block above
                if hasattr(full_chat, "full_chat") and hasattr(full_chat.full_chat, "invite_link"):  # type: ignore  # Telethon returns typed result
                    return full_chat.full_chat.invite_link or "No invite link available."  # type: ignore  # Telethon returns typed result
...
async def join_chat_by_link(link: str) -> str:
    ...
    try:
        ...
        try:
            ...
            if hasattr(invite_info, "chat") and invite_info.chat:  # type: ignore  # Telethon returns typed result
                # If we got chat info, we're already a member
                chat_title = getattr(invite_info.chat, "title", "Unknown Chat")  # type: ignore  # Telethon returns typed result
        ...
        if result and hasattr(result, "chats") and result.chats:  # type: ignore  # Telethon returns typed result
            chat_title = getattr(result.chats[0], "title", "Unknown Chat")  # type: ignore  # Telethon returns typed result
...
@validate_id("chat_id")
async def export_chat_invite(chat_id: int | str) -> str:
    """
...
@validate_id("chat_id")
async def send_voice(chat_id: int | str, file_path: str) -> str:
    """
    ...
    try:
        ...
        entity = await client.get_entity(chat_id)
        await client.send_file(entity, file_path, voice_note=True)  # type: ignore  # Telethon accepts this at runtime
...
async def forward_message(from_chat_id: int | str, message_id: int, to_chat_id: int | str) -> str:
    """
    ...
    try:
        ...
        await client.forward_messages(to_entity, message_id, from_entity)  # type: ignore  # Telethon accepts this at runtime
...
@validate_id("chat_id")
async def edit_message(chat_id: int | str, message_id: int, new_text: str) -> str:
    """
    Edit a message you sent.
    """
    try:
        entity = await client.get_entity(chat_id)
        await client.edit_message(entity, message_id, new_text)  # type: ignore  # Telethon accepts this at runtime
...
async def delete_message(chat_id: int | str, message_id: int) -> str:
    ...
    try:
        entity = await client.get_entity(chat_id)
        await client.delete_messages(entity, message_id)  # type: ignore  # Telethon accepts this at runtime
...
@validate_id("chat_id")
async def pin_message(chat_id: int | str, message_id: int) -> str:
    """
    Pin a message in a chat.
    """
    try:
        entity = await client.get_entity(chat_id)
        await client.pin_message(entity, message_id)  # type: ignore  # Telethon accepts this at runtime
...
@validate_id("chat_id")
async def unpin_message(chat_id: int | str, message_id: int) -> str:
    """
    Unpin a message in a chat.
    """
    try:
        entity = await client.get_entity(chat_id)
        await client.unpin_message(entity, message_id)  # type: ignore  # Telethon accepts this at runtime
...
@validate_id("chat_id")
async def mark_as_read(chat_id: int | str) -> str:
    """
    Mark all messages as read in a chat.
    """
    try:
        entity = await client.get_entity(chat_id)
        await client.send_read_acknowledge(entity)  # type: ignore  # Telethon accepts this at runtime
...
@validate_id("chat_id")
async def reply_to_message(chat_id: int | str, message_id: int, text: str) -> str:
    """
    Reply to a specific message in a chat.
    """
    try:
        entity = await client.get_entity(chat_id)
        await client.send_message(entity, text, reply_to=message_id)  # type: ignore  # Telethon accepts this at runtime
...
@validate_id("chat_id")
async def get_media_info(chat_id: int | str, message_id: int) -> str:
    """
    ...
    try:
        ...
        return str(msg.media)  # type: ignore  # Telethon returns typed result
    except Exception as e:
...
async def search_messages(chat_id: int | str, query: str, limit: int = 20) -> str:
    ...
    try:
        ...
        lines = []
        for msg in messages:  # type: ignore  # Telethon returns typed result
            sender_name = get_sender_name(msg)
            reply_info = ""
            if msg.reply_to and msg.reply_to.reply_to_msg_id:
                reply_info = f" | reply to {msg.reply_to.reply_to_msg_id}"
...
@validate_id("chat_id")
async def mute_chat(chat_id: int | str) -> str:
    """
    ...
    try:
        ...
        await client(
            functions.account.UpdateNotifySettingsRequest(
                peer=peer,  # type: ignore[arg-type]  # Telethon accepts entities at runtime
                settings=InputPeerNotifySettings(mute_until=datetime(2038, 1, 1)),
            )
        )
        ...
        try:
            ...
            peer = await client.get_input_entity(chat_id)  # type: ignore  # reassigning peer to input entity type
            await client(
                functions.account.UpdateNotifySettingsRequest(
                    peer=peer,  # type: ignore  # Telethon accepts this at runtime
                    settings={  # type: ignore  # Telethon accepts this at runtime
...
@validate_id("chat_id")
async def unmute_chat(chat_id: int | str) -> str:
    """
    ...
    try:
        ...
        peer = await client.get_entity(chat_id)
        await client(
            functions.account.UpdateNotifySettingsRequest(peer=peer, settings=InputPeerNotifySettings(mute_until=0))  # type: ignore  # Telethon accepts this at runtime
        )
...
@validate_id("chat_id")
async def archive_chat(chat_id: int | str) -> str:
    """
    Archive a chat.
    """
    try:
        await client(functions.messages.ToggleDialogPinRequest(peer=await client.get_entity(chat_id), pinned=True))  # type: ignore  # Telethon accepts this at runtime
...
@validate_id("chat_id")
async def unarchive_chat(chat_id: int | str) -> str:
    """
    Unarchive a chat.
    """
    try:
        await client(functions.messages.ToggleDialogPinRequest(peer=await client.get_entity(chat_id), pinned=False))  # type: ignore  # Telethon accepts this at runtime
...
async def get_sticker_sets() -> str:
    ...
    try:
        ...
        return json.dumps([s.title for s in result.sets], indent=2)  # type: ignore  # Telethon returns typed result
    except Exception as e:
...
@validate_id("chat_id")
async def send_sticker(chat_id: int | str, file_path: str) -> str:
    """
    ...
    try:
        ...
        entity = await client.get_entity(chat_id)
        await client.send_file(entity, file_path, force_document=False)  # type: ignore  # Telethon accepts this at runtime
...
async def get_gif_search(query: str, limit: int = 10) -> str:
    ...
    try:
        ...
        try:
            result = await client(functions.messages.SearchGifsRequest(q=query, offset_id=0, limit=limit))  # type: ignore  # Telethon returns typed result
            if not result.gifs:  # type: ignore  # Telethon returns typed result
                return "[]"
            return json.dumps([g.document.id for g in result.gifs], indent=2, default=json_serializer)  # type: ignore  # Telethon returns typed result
            ...
            try:
                ...
                result = await client(
                    functions.messages.SearchRequest(
                        peer="gif",  # type: ignore  # Telethon accepts this at runtime
                        q=query,
                ...
                if not result or not hasattr(result, "messages") or not result.messages:  # type: ignore  # Telethon returns typed result
                    return "[]"
                # Extract document IDs from any messages with media
                gif_ids = []
                for msg in result.messages:  # type: ignore  # Telethon returns typed result
...
@validate_id("chat_id")
async def send_gif(chat_id: int | str, gif_id: int) -> str:
    """
    ...
    try:
        ...
        entity = await client.get_entity(chat_id)
        await client.send_file(entity, gif_id)  # type: ignore  # Telethon accepts this at runtime
...
async def get_bot_info(bot_username: str) -> str:
    ...
    try:
        ...
        result = await client(functions.users.GetFullUserRequest(id=entity))  # type: ignore  # Telethon accepts this at runtime

        # Create a more structured, serializable response
        if hasattr(result, "to_dict"):
            # Use custom serializer to handle non-serializable types
            return json.dumps(result.to_dict(), indent=2, default=json_serializer)  # type: ignore  # Telethon returns typed result
        else:
            # Fallback if to_dict is not available
            info = {
                "bot_info": {
                    "id": entity.id,  # type: ignore  # entity type narrowed by hasattr/isinstance
                    "username": entity.username,  # type: ignore  # entity type narrowed by hasattr/isinstance
                    "first_name": entity.first_name,  # type: ignore  # entity type narrowed by hasattr/isinstance
                    "last_name": getattr(entity, "last_name", ""),
                    "is_bot": getattr(entity, "bot", False),
                    "verified": getattr(entity, "verified", False),
                }
            }
            if hasattr(result, "full_user") and hasattr(result.full_user, "about"):  # type: ignore  # Telethon returns typed result
                info["bot_info"]["about"] = result.full_user.about  # type: ignore  # Telethon returns typed result
...
async def set_bot_commands(bot_username: str, commands: list) -> str:
    ...
    try:
        ...
        if not getattr(me, "bot", False):
            return (
                "Error: This function can only be used by bot accounts. "
...
@validate_id("chat_id")
async def get_history(chat_id: int | str, limit: int = 100) -> str:
    """
...
@validate_id("user_id")
async def get_user_photos(user_id: int | str, limit: int = 10) -> str:
    """
    Get profile photos of a user.
    """
    try:
        user = await client.get_entity(user_id)
        photos = await client(functions.photos.GetUserPhotosRequest(user_id=user, offset=0, max_id=0, limit=limit))  # type: ignore  # Telethon accepts this at runtime
        return json.dumps([p.id for p in photos.photos], indent=2)  # type: ignore  # Telethon returns typed result
    except Exception as e:
...
@validate_id("user_id")
async def get_user_status(user_id: int | str) -> str:
    """
    Get the online status of a user.
    """
    try:
        user = await client.get_entity(user_id)
        return str(user.status)  # type: ignore  # entity type narrowed by hasattr/isinstance
    except Exception as e:
...
@validate_id("chat_id")
async def get_recent_actions(chat_id: int | str) -> str:
    """
    ...
    try:
        result = await client(
            functions.channels.GetAdminLogRequest(
                channel=chat_id,  # type: ignore  # Telethon accepts this at runtime
                q="",
                events_filter=None,
                admins=[],
                max_id=0,
        ...
        if not result or not result.events:  # type: ignore  # Telethon returns typed result
            return "No recent admin actions found."

        # Use the custom serializer to handle datetime objects
        return json.dumps([e.to_dict() for e in result.events], indent=2, default=json_serializer)  # type: ignore  # Telethon returns typed result
    except Exception as e:
...
@validate_id("chat_id")
async def get_pinned_messages(chat_id: int | str) -> str:
    """
    ...
    try:
        ...
        try:
            ...
            messages = [m for m in all_messages if getattr(m, "pinned", False)]  # type: ignore  # Telethon returns typed result
        ...
        for msg in messages:  # type: ignore  # Telethon returns typed result
            ...
            if msg.reply_to and msg.reply_to.reply_to_msg_id:
                reply_info = f" | reply to {msg.reply_to.reply_to_msg_id}"
            lines.append(
                f"ID: {msg.id} | {sender_name} | Date: {msg.date}{reply_info}"
                f" | Message: {msg.message or '[Media/No text]'}"
            )
    ...
    try:
        ...
        await client(
            functions.messages.SendMediaRequest(
                peer=entity,  # type: ignore  # Telethon accepts this at runtime
...
async def get_message_reactions(
    ...
    try:
        ...
        if not result.reactions:  # type: ignore  # Telethon returns typed result
            return f"No reactions on message {message_id} in chat {chat_id}."

        reactions_data = []
        for reaction in result.reactions:  # type: ignore  # Telethon returns typed result
...
async def get_drafts() -> str:
    ...
    try:
        ...
        if hasattr(result, "updates"):
            for update in result.updates:  # type: ignore  # Telethon returns typed result
...
@validate_id("chat_id")
async def clear_draft(chat_id: int | str) -> str:
    """
...
async def _main() -> None:
    try:
        ...
        await telethon_client.start(client)
...
if __name__ == "__main__":
    # Accept "server" subcommand for Tauri compatibility, or run directly
    if len(sys.argv) > 1 and sys.argv[1].lower() == "server":
        sys.argv.pop(1)  # Remove "server" so MCP doesn't see it
    main()
