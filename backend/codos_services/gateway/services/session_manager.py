...
class SessionManager:
    ...
    def create_session(self, session_id: str, agent_id: str = "engineer") -> dict:
        ...
        env = os.environ.copy()
        env["VAULT_PATH"] = str(settings.get_vault_path())
        env["CODOS_PATH"] = str(settings.get_codos_path())
...
[User]: {current_message}"""
    ...
    def send_message(self, session_id: str, message: str, history: list[dict] | None = None) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                logger.warning(
                    f"Failed to send keys: "
                    f"{result.stderr.decode() if isinstance(result.stderr, bytes) else result.stderr}"
                )
                return False
            ...
            if not success:
                logger.warning(
                    f"Failed to send Enter to {tmux_name}: "
                    f"{result.stderr.decode() if isinstance(result.stderr, bytes) else result.stderr}"
                )
