...
def build_attachment_contexts(session_id: str, attachments_payload: Any) -> list[AttachmentContext]:
    ...
    for raw_item in attachments_payload:
        ...
        name: str = raw_name if isinstance(raw_name, str) else resolved_path.name
        raw_attachment_id = raw_item.get("attachment_id")
        attachment_id: str = (
            raw_attachment_id if isinstance(raw_attachment_id, str) else resolved_path.stem.split("_", 1)[0]
        )
        raw_mime = raw_item.get("mime")
        mime: str = raw_mime if isinstance(raw_mime, str) else _guess_mime(name, None)
