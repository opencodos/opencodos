...
from backend.codos_models.settings import SESSIONS_DIR, settings
    ...
    async def send_prompt(
        ...
        if self._claude_path is None:
            ...
            if self._claude_path is None:
                yield {
                    "type": "error",
                    "error": "claude CLI not found. Please install: curl -fsSL https://claude.ai/install.sh | bash",
                }
                return
                        ...
                        try:
                            ...
                            if event.get("type") == "result":
                                if event.get("is_error"):
                                    ...
                                    # Intercept auth errors: emit one clean message instead of
                                    # the raw result (which produces duplicate messages)
                                    auth_keywords = (
                                        "not logged in",
                                        "please run /login",
                                    ...
                                    if any(kw in error_text.lower() for kw in auth_keywords):
                                        yield {
                                            "type": "error",
                                            "error": (
                                                "Claude is not logged in. Please run "
                                                "`claude auth login` in your terminal, "
                                                "then reload the page."
                                            ),
                                        }
                                        return
                                    # Non-auth error: suppress raw result event (event_translator
                                    # would flush its text as a visible message), emit only the error
                                    yield {
                                        "type": "error",
                                        "error": error_text,
                                    }
