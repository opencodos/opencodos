...
def _get_telegram_config_path() -> Path:
    """Get Telegram config.yaml path."""
    return settings.get_telegram_config_path()
...
def _save_telegram_filters(codos_path: Path, filters: TelegramFilterConfig) -> None:
    ...
    sync_section: dict = raw["sync"]
    sync_section["sync_unread_only"] = filters.sync_unread_only
    sync_section["include_dms"] = filters.include_dms
    sync_section["include_groups"] = filters.include_groups
    sync_section["include_channels"] = filters.include_channels
    sync_section["include_muted"] = filters.include_muted
    sync_section["include_archived"] = filters.include_archived
    sync_section["mark_unread_after_sync"] = filters.mark_unread_after_sync
...
def _generate_plist(connector: str, preset: SchedulePreset, codos_path: Path) -> str:
    ...
    if runtime == Runtime.BUN:
        ...
        # Dev mode Python — use canonical venv python path
        program = str(settings.get_backend_venv_python())
        program_args = args

    # Create log directory in a writable location
    log_dir = Path(settings.codos_data_dir) / "logs" / f"{connector}-sync"
...
{chr(10).join(calendar_entries)}
    ...
    env_vars_xml = f"""        <key>PATH</key>
        <string>{str(Path.home())}/.bun/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin</string>
        <key>CODOS_DATA_DIR</key>
        <string>{settings.codos_data_dir}</string>"""
...
def _get_schedule_description(preset: SchedulePreset) -> str:
    ...
    if preset.id == "manual":
        ...
        if len(hour_parts) > 1:
            return f"Daily at {', '.join(f'{h}:{minute.zfill(2)}' for h in hour_parts)}"
        else:
