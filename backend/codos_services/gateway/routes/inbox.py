...
_MY_PATTERNS = tuple(p.strip().lower() for p in os.getenv("OWNER_PATTERNS", "").split(",") if p.strip())
...
async def send_telegram_message(request: SendMessageRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="Telegram agent not reachable") from None
