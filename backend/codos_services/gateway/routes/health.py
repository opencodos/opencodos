...
SERVICES: list[ServiceDef] = [
    {"name": "gateway-backend", "port": 8767},
...
JOB_LOG_DIRS = {
    ...
    "com.codos.codos-bot": "codos-bot",
    "com.codos.atlas-alerts": "atlas-alerts",
}
...
def _resolve_log_path(codos_path: Path, log_dir: str, filename: str) -> Path | None:
    ...
    candidates = [
        Path(settings.codos_data_dir) / "logs" / log_dir / filename,
...
def _is_connector_configured(connector: str, codos_path: Path) -> bool:
    ...
    if connector == "telegram":
        return settings.get_telegram_session_path().exists()
...
def _load_sync_task_last_success_by_connector() -> dict[str, datetime]:
    ...
    sync_tasks_path = Path(settings.codos_data_dir) / "sync-tasks.json"
...
def _get_atlas_jobs(codos_path: Path, sync_fallback: dict[str, datetime] | None = None) -> list[JobHealth]:
    ...
    try:
        ...
        for line in result.stdout.splitlines():
            # Include both com.codos.* and com.codos.* jobs
            if "com.codos." not in line:
                continue
...
def _get_connector_freshness(
    codos_path: Path, sync_fallback: dict[str, datetime] | None = None
