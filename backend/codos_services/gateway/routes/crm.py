...
class Contact(BaseModel):
    ...
    deal_stage: str | None = (
        None  # first_contact | call | negotiation | signed | stale | disqualified | closed_won | closed_lost
    )
...
def _load_contacts() -> dict:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to parse contacts YAML: {str(e)}") from e
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to load contacts: {str(e)}") from e
...
def _save_contacts(data: dict) -> None:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to save contacts: {str(e)}") from e
