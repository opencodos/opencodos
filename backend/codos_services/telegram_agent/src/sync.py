...
from backend.codos_services.telegram_agent.crm_update import find_contact, load_crm
from backend.codos_services.telegram_agent.src.config import Config, add_to_pending, add_to_whitelist, save_whitelist
from backend.codos_services.telegram_agent.src.obsidian import ObsidianWriter
from backend.codos_services.telegram_agent.src.telegram_client import Conversation, TelegramClientWrapper
    ...
    def _write_unread_summary(self, conversations: list[Conversation]) -> None:
        ...
        if not unread:
            ...
            summary_path.write_text(
                json.dumps({"conversations": [], "generated": datetime.now(UTC).isoformat()}, indent=2)
            )
            return
        ...
        async def sync_single(conv: Conversation) -> tuple:
            ...
            async with self._semaphore:
                ...
                try:
                    ...
                    count = await self._sync_conversation(
                        conv, checkpoints, matched_contact, match_confidence, context_count=context_count
                    )
