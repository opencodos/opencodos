...
PRIORITY_CONFIG = CODOS_PATH / "backend/codos_services/telegram_agent/priority_contacts.yaml"
...
def load_contact_mapping() -> dict:
    ...
    if not CONTACTS_PATH.exists():
        return {
            "contact_to_projects": {},
            "personal_contacts": [],
            "projects": os.environ.get("PROJECT_LIST", "Project A,Project B,Project C").split(","),
        }
    ...
    for contact in data.get("contacts", []):
        ...
        if projects:
            contact_to_projects = mapping["contact_to_projects"]
            if isinstance(contact_to_projects, dict):
                contact_to_projects[name] = projects

        # Track personal contacts
        if contact.get("category") == "personal":
            personal_contacts = mapping["personal_contacts"]
            if isinstance(personal_contacts, list):
                personal_contacts.append(name)
...
BEFORE writing the summary, scan ALL messages for these crisis patterns. \
If ANY match, include them in a "## CRISIS SIGNALS" section at the TOP of output (before Executive Summary).
...
CRITICAL: Before writing the project sections, extract ALL messages where the user needs to respond. \
...
### Executive Summary
2-3 bullets of what actually matters for the priorities above. \
If nothing important happened, say "No significant updates in the last 24 hours."
