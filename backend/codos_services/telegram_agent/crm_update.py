...
def find_contact_by_telegram_id(crm: dict, telegram_id: int) -> dict[str, Any] | None:
    """Find a contact by exact telegram_id match only."""
    if not telegram_id:
        return None
    for contact in crm.get("contacts", []):
        if contact.get("telegram_id") == telegram_id:
            result: dict[str, Any] = contact
            return result
    return None


def find_contact(
    crm: dict, telegram_id: int | None = None, name: str | None = None
...
def gather_slack_interactions() -> list[dict]:
    ...
    for md_file in SLACK_FOLDER.glob(f"{today}*.md"):
        ...
        senders_seen: dict[str, list[str]] = {}  # sender -> list of messages
...
def main():
    ...
    # Generate dashboard
    from backend.codos_services.telegram_agent.crm_dashboard import generate_dashboard
