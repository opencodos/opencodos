...
if [ -d "$CODOS_ROOT/.venv" ]; then
    source "$CODOS_ROOT/.venv/bin/activate"
fi
...
# --strict-mcp-config without --mcp-config = load zero MCP servers (prevents hangs)
run_with_timeout $CLAUDE_TIMEOUT claude -p "$CLAUDE_PROMPT" \
    --model claude-opus-4-6 \
    --allowedTools "Read,Write" \
    --permission-mode bypassPermissions \
    --strict-mcp-config \
    2>&1 | tee /tmp/telegram-summary-generation.log
...
if [ "$CLAUDE_EXIT" -eq 0 ]; then
    ...
    echo ""
    echo "Error: Claude Code timed out or failed (exit $CLAUDE_EXIT)"
