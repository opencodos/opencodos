...
def main():
    ...
    if command == "server":
        ...
        log_file = str(settings.get_log_dir() / "codos-telegram.log")
        configure_logging("telegram-agent", log_file=log_file)

        import uvicorn

        from backend.codos_services.telegram_agent.server import app
        ...
        from backend.codos_services.telegram_agent.agent import main as agent_main
